...
async def registerAgent(text_array):
    if len(text_array) == 1:
        ...
        response = "END Invalid input."
    return response
async def registerMember(text_array):
    if len(text_array) == 1:
        ...
        company_code = text_array[1]
        agent = text_array[2]
        county = text_array[3]
        phone = text_array[4]
        surname = text_array[5]
        other_names = text_array[6]
        ...
        member = Member(
            ...
            AgentId = agent['id'],OtherNames = other_names,
...
async def addBeneficiary(text_array,user=None):
    if len(text_array) == 4:
...
async def loanApplication(text_array,initial=0,user=None):
    ...
    if len(text_array) > initial:
        ...
        if len(text_array) - initial == 2:
            ...
            if float(text_array[initial]) > loan_limit:
                ...
                response += f'{selected_type['LoanType']} LOAN for amount {text_array[initial]} and repayment period of {selected_type['RepayPeriod']} Months and Interest rate {selected_type['Interest']}% has been recieved.\n1.Confirm\n2.Cancel\n'
        if len(text_array) - initial == 3:
            ...
            if text_array[-1] == '1':
                ...
                new_loan = Loan(
                    ...
                    RepayPeriod = selected_type['RepayPeriod'],
                    IdNo = user['IDNo'],
    ...
    response += '#.Previous Menu\n00.Main Menu.\n'
    return response
async def loanGuarantors(text_array,initial=0,user=None):
...
async def myShares(text_array,initial=0,user=None):
...
async def myLoans(text_array,initial=0,user=None):
...
async def MemberWithdrawal(text_array,initial=0,user=None):
