...
class Agents:
    ...
    async def find(id):
        ...
        agent = Agent(
            ...
            MobileNo = agent[7],branchName = agent[8],
            CompanyCode = agent[9],HomeAddress = agent[10],
            Town = agent[11],PIN=agent[-1]
        )
        return agent.get()
    @staticmethod
    async def updatePIN(agent):
        query = DatabaseManager.update(f"update Agents set PIN='%s' where IdNo='%s' and id='%s'"%(agent["PIN"],agent['IdNo'],agent['id']))
